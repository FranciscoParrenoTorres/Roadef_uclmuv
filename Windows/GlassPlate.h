...
class GlassPlate
{
    public:
        GlassPlate();
		GlassPlate(int, GlassRectangle &, int, unsigned int *);
		GlassPlate(int nodoant, std::vector<int> &vecp, int nplate, unsigned int *nodos, int ntype, unsigned int ncut, int);

		GlassPlate(GlassRectangle & R,int , unsigned int * );
        virtual ~GlassPlate();
    ...
    private:
        ...
        int type;   /* node type
                     ...
                     */
        unsigned int cut; // Cut stage (Note: since this is plate class, plate's instance cut G_level should be 0-cut).
        ...
        unsigned int successor_nbr; // Plate's successor nodes number (G_children).
